...
class ListAdapter (val con: Context, val layoutResId: Int, val list: List<Users>)
    ...
    private fun showUpdateDialog(user: Users) {
        ...
        view.edt_nama_update.setText(user.nama)
        view.edt_email_update.setText(user.email)

        builder.setView(view)
        builder.setPositiveButton("Update"){dialogInterface, i ->
            val dbUsers = FirebaseDatabase.getInstance().getReference("USERS")
            val nama = view.edt_nama_update.text.toString().trim()
            val email = view.edt_email_update.text.toString().trim()

            when{
                nama.isEmpty() -> {
                    view.edt_nama_update.error = "Nama Tidak Boleh Kosong"
                    return@setPositiveButton
                }
                email.isEmpty() -> {
                    view.edt_email_update.error = "Email Tidak Boleh Kosong"
                    return@setPositiveButton
                }
                else -> {
